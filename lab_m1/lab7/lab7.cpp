...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int matShininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(matShininess, materialShininess);

    int matkd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(matkd, materialKd);

    int matks = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(matks, materialKs);

    int objColor = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(objColor, 1, glm::value_ptr(color));
